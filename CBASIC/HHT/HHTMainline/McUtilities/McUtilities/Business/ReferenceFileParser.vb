...
Public Class ReferenceFileParser
    ...
    Sub New()
        ...
        CONN_STRING = "Data Source = " & objAppContainer.objConfigFileParams.DB
    ...
    Public Function ParseFile(ByVal strFileName As String, Optional ByVal iIndex As Integer = 0) As Boolean
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseFile:: Parsing file " & strFileName, _
                                           Logger.LogLevel.RELEASE)
        Dim rdrFileReader As IO.StreamReader = Nothing
        Try
            Dim strPath As String = objAppContainer.objConfigFileParams.TFTP.LocalFilePath.TrimEnd("\") + "\"
        ...
        Try
            'Start DBUpdation process.
            ReferenceDataMgr.GetInstance.objDownloadReferenceData.DBUpdationStart()
            Select Case strFileName
                Case objAppContainer.objConfigFileParams.BOOTCODE
                    Return Me.ParseBootCodeFile(strFileName, _
                                                objAppContainer.objConfigFileParams.TFTP.LocalFilePath, iIndex)
                    Exit Select
            ...
            End Select
            'Start DBUpdation process.
            ReferenceDataMgr.GetInstance.objDownloadReferenceData.DBUpdationStop()
        Catch ex As Exception
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: Parsefile :: Parsing " & strFileName & _
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function PurgeReferenceData(ByVal strTableName As String) As Boolean
        ...
        Try
            ...
            strInsertCmd = "DELETE FROM " & strTableName
        ...
        Catch ex As Exception
            ...
            objAppContainer.objLogger.WriteAppLog("RefFileParser: Exception in purging the existing data from " _
                                 & strTableName & ex.Message, Logger.LogLevel.RELEASE)
        End Try
    End Function
    ''' <summary>
    ...
    Private Function ParseBootCodeFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseBootCodeFile::File parsing started for BOOTCODE.CSV.", _
                                                        Logger.LogLevel.RELEASE)
        ...
        Try
            ...
            While Not strLine.StartsWith("T")
                ...
                If iCount = m_PercentageValue Then
                    ReferenceDataMgr.GetInstance.objDownloadReferenceData.SetDBUpdationstatus(iPercentage)
                ...
                If strLine.StartsWith("D") Then
                    Try
                        ...
                        If staData.Length <> 67 Then
                            iCount = iCount + 1
                            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseBootCodeFile::Error - Length mismatch for BOOTCODE.CSV.", _
                                                        Logger.LogLevel.RELEASE)
                    ...
                    Catch ex As Exception
                        ...
                        If Not ex.Message.Contains("duplicate") Then
                            objAppContainer.objLogger.WriteAppLog("RefFileParser: Error " _
                ...
                ElseIf strLine.StartsWith("H") Then
                    If Not strLine.Contains(strFileName) Then
                        objAppContainer.objLogger.WriteAppLog("RefFileParser: Invalid file format. The " _
        ...
        Catch ex As FileNotFoundException
            objAppContainer.objLogger.WriteAppLog("RefFileParser: File" & strFileName & "not found.", _
                                                  Logger.LogLevel.RELEASE)
            ...
            'close the reader
            rdrFileReader.Close()
            'Updating the error Message in XML
            BatchConfigParser.GetInstance().UpdateDBPopulationIndex(strFileName, iCount, ex.Message)
        ...
        'Return true.
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseBootCodeFile::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
    ...
    Private Function ParseBarCode(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseBarCode::File parsing started for BARCODE.CSV.", _
                                                        Logger.LogLevel.RELEASE)
            ...
            While Not strLine.StartsWith("T")
                ...
                If strLine.StartsWith("D") Then
                    ...
                    If staData.Length <> 2 Then
                        iCount = iCount + 1
                        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseBarCodeFile::Error - Length mismatch for BOOTCODE.CSV.", _
                                                        Logger.LogLevel.RELEASE)
                        If Not bIsErrorLOGSend Then
                    ...
                    'Minu Removed single quotes from the query to insert single quotes
                    ...
                    Catch ex As Exception
                        'Log the duplicate record for reference.
                        If Not ex.Message.Contains("duplicate") Then
                            objAppContainer.objLogger.WriteAppLog("RefFileParser: duplicate " _
                                                                  & "record found " + strLine, _
                                                                  Logger.LogLevel.INFO)
                        End If
                        If Not bIsErrorLOGSend Then
        ...
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseBarCode::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.BARCODE, _
    ...
    Private Function ParseDealFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseDealFile::File parsing started for DEAL.CSV.", _
                                                        Logger.LogLevel.RELEASE)
            ...
            While Not strLine.StartsWith("T")
                ...
                ElseIf strLine.StartsWith("H") Then
                    If Not strLine.Contains(strFileName) Then
                        ...
                        objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                             LOGTransmitter.Status.ABEND, LOGTransmitter.FileName.DEAL, LOGTransmitter.Reasons.Header_Name_Mismatch)
                        'return status
                        Return False
                        bIsErrorLOGSend = True
                    End If
        ...
        Catch ex As Exception
            'Catch the exception and write the stack trace.
            objAppContainer.objLogger.WriteAppLog("RefFileParser: Exception occurred" & _
                                                  ex.StackTrace, Logger.LogLevel.RELEASE)
            'Error Occured - send a LOG Message 
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
        ...
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseDealFile::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.DEAL, _
    ...
    Private Function ParsePGroupFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParsePGroupFile::File parsing started for PGROUP.CSV.", _
                                                        Logger.LogLevel.RELEASE)
        ...
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParsePGroupFile::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.PGROUP, _
    ...
    Private Function ParseSupplierFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseSupplierFile::File parsing started for SUPPLIER.CSV.", _
                                                        Logger.LogLevel.RELEASE)
        'Send  a log message
        objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                  LOGTransmitter.Status.START, _
        ...
        Try
            ...
            BatchConfigParser.GetInstance().UpdateDBPopulationIndex(strFileName, iCount)
            'AppContainer.GetInstance.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
            '                                                    LOGTransmitter.Status.END_OK, LOGTransmitter.FileName.SUPPLIER, LOGTransmitter.Reasons.Parse_Complete)
            'Create index and statistics.
        ...
        'Successful send a LOG message
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseSupplierFile::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.SUPPLIER, _
    ...
    Private Function ParseUsersFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseUsersFile::File parsing started for USERS.CSV.", _
                                                        Logger.LogLevel.RELEASE)
        'Send a LOG message
        objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                   LOGTransmitter.Status.START, _
        ...
        Try
            ...
            While Not strLine.StartsWith("T")
                ...
                If strLine.StartsWith("D") Then
                    ...
                    'Escape the single quotes characters if any present in the user's name.
                    staData(2) = staData(2).Replace("'", "''")
            ...
            BatchConfigParser.GetInstance().UpdateDBPopulationIndex(strFileName, iCount)
            'AppContainer.GetInstance.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
            '                                                    LOGTransmitter.Status.END_OK, LOGTransmitter.FileName.USERS, LOGTransmitter.Reasons.Parse_Complete)
            'Create index and statistics.
        ...
        'Return true.
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseUsersFile::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.USERS, _
    ...
    Private Function ParseRecallFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseRecallFile::File parsing started for RECALL.CSV.", _
                                                        Logger.LogLevel.RELEASE)
        ...
        Try
            ...
            While Not strLine.StartsWith("T")
                ...
                If strLine.StartsWith("D") Then
                    ...
                    staData = strLine.Split(DELIMETER)
                    'Check for length of the data.
                    objAppContainer.objLogger.WriteAppLog("RefFileParser:: " + strLine, _
                                                    Logger.LogLevel.RELEASE)
            ...
            BatchConfigParser.GetInstance().UpdateDBPopulationIndex(strFileName, iCount)
            'AppContainer.GetInstance.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
            '                                                    LOGTransmitter.Status.END_OK, LOGTransmitter.FileName.RECALL, LOGTransmitter.Reasons.Parse_Complete)
            'Create index and statistics.
        ...
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseRecallFile::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.RECALL, _
    ...
    Private Function ParseLivePOGFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseLivePOGFile::File parsing started for LIVEPOG.CSV.", _
                                                        Logger.LogLevel.RELEASE)
        'Starting to parse - send a LOG message
        objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                  LOGTransmitter.Status.START, _
        ...
        Try
            ...
            While Not strLine.StartsWith("T")
                ...
                If strLine.StartsWith("D") Then
                    ...
                    If staData.Length <> 8 Then
                        iCount = iCount + 1
                        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseLivePOGFile::Error - Length mismatch: " & strLine, _
                                                        Logger.LogLevel.RELEASE)
                        If Not bIsErrorLOGSend Then
            ...
            BatchConfigParser.GetInstance().UpdateDBPopulationIndex(strFileName, iCount)
            'AppContainer.GetInstance.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
            '                                                    LOGTransmitter.Status.END_OK, LOGTransmitter.FileName.LIVEPOG, LOGTransmitter.Reasons.Parse_Complete)
            'Create index and statistics.
        ...
        'Successful - send a LOG message
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseLivePOGFile:: DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.LIVEPOG, _
    ...
    Private Function ParseCategoryFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseCategoryFile::File parsing started for CATEGORY.CSV.", _
                                                        Logger.LogLevel.RELEASE)
        ...
        Try
            ...
            While Not strLine.StartsWith("T")
                ...
                If strLine.StartsWith("D") Then
                    ...
                    If staData.Length <> 4 Then
                        iCount = iCount + 1
                        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseCategoryFile::Error - Length mismatch: " & strLine, _
                                                        Logger.LogLevel.RELEASE)
                        If Not bIsErrorLOGSend Then
            ...
            BatchConfigParser.GetInstance().UpdateDBPopulationIndex(strFileName, iCount)
            'AppContainer.GetInstance.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
            '                                                    LOGTransmitter.Status.END_OK, LOGTransmitter.FileName.CATEGORY, LOGTransmitter.Reasons.Parse_Complete)
        ...
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseCategoryFile::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.CATEGORY, _
    ...
    Private Function ParseModuleFile(ByVal strFileName As String, ByVal strPath As String, Optional ByVal iFileIndex As Integer = 0) As Boolean
        ...
        Dim iCount As Integer = 0
        Dim iPercentage As Integer = 10
        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseModuleFile::File parsing started for MODULE.CSV.", _
                                                        Logger.LogLevel.RELEASE)
        ...
        Try
            ...
            While Not strLine.StartsWith("T")
                ...
                If strLine.StartsWith("D") Then
                    ...
                    ''Check LEngth oF the data changed , Data length - 6
                    If staData.Length <> 6 Then
                        iCount = iCount + 1
                        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseModuleFile::Error - Length mismatch: " & strLine, _
                                                        Logger.LogLevel.RELEASE)
                        If Not bIsErrorLOGSend Then
                            'Length Mismatch - Send LOG
                            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
            ...
            BatchConfigParser.GetInstance().UpdateDBPopulationIndex(strFileName, iCount)
            'AppContainer.GetInstance.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
            '                                                    LOGTransmitter.Status.END_OK, LOGTransmitter.FileName.MODULES, LOGTransmitter.Reasons.Parse_Complete)
            'Create index and statistics.
        ...
        'SuccesFul Send LOG
        If Not bIsErrorLOGSend Then
            objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseModuleFile::DB update successful for " + strFileName, _
                                                            Logger.LogLevel.RELEASE)
            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
                                                                      LOGTransmitter.Status.END_OK, _
                                                                      LOGTransmitter.FileName.MODULES, _
        ...
        Try
            ...
            While Not strLine.StartsWith("T")
                ...
                If strLine.StartsWith("D") Then
                    ...
                    If staData.Length <> 2 Then
                        iCount = iCount + 1
                        objAppContainer.objLogger.WriteAppLog("RefFileParser:: ParseShelFDescFile::Error - Length mismatch: " & strLine, _
                                                        Logger.LogLevel.RELEASE)
                        If Not bIsErrorLOGSend Then
                            'Length Mismatch - Send LOG
                            objAppContainer.objLogMessageTransmitter.sendLog(LOGTransmitter.Action.TFTP_LOAD, _
    ...
    ''' <returns>Boolean</returns>
    ''' <remarks></remarks>
    Public Function ExecuteSampleQuery(ByVal strFile As String) As Boolean
        Dim strInsertCmd As String = ""
        Dim strLine As String = Nothing
        Dim iCount As Integer = 0
            ...
            Select Case strFile
                ...
                Case Else
                    sqlQuery = "SELECT BarCodeView.Boots_Code, BootsCodeView.Item_Status, " _
                        ...
                        & "WHERE (BarCodeView.BarCode = '1234567')"
            End Select
            'Executing the querry
        ...
        Catch ex As Exception
            'Log the duplicate record for reference.
            objAppContainer.objLogger.WriteAppLog("RefFileParser: Error record found " + strLine, _
                                Logger.LogLevel.INFO)
        End Try
        'If successfully inserted the entire list of file contents to DB
        'Return true.
    ...
    Private Sub DeleteIndex(ByVal strTableName As String)
        ...
        Catch ex As Exception
            'Log the duplicate record for reference.
            objAppContainer.objLogger.WriteAppLog("RefFileParser: DeleteIndex: " + ex.Message, _
    ...
    Private Sub CreateIndex(ByRef strTableName As String, ByVal strColumns As String, Optional ByVal strIndex As String = "NA")
        ...
        Dim iVal As Integer = 0
        Try
            objAppContainer.objLogger.WriteAppLog("Create Index: " + Now.ToString(), Logger.LogLevel.RELEASE)
            Dim sqlCommd As New SqlCeCommand()
            'Dim sqlRdr As SqlCeDataReader
        ...
        Catch ex As Exception
            'Log the duplicate record for reference.
            objAppContainer.objLogger.WriteAppLog("RefFileParser: Create Index: " + ex.Message, _
                                                         Logger.LogLevel.RELEASE)
        End Try
    End Sub
    ''' <summary>
    ...
    Private Sub CreateStatistics(ByVal strTableName As String, ByVal strColumns As String)
        ...
        Try
            ...
            strCreateStats = String.Format("CREATE STATISTICS {0} ON {1}({2}) WITH FULLSCAN, NORECOMPUTE", strStats, strTableName, strColumns)
            'Executing the querry
            sqlCommd = New SqlCeCommand(strCreateStats, sqlConn)
            iVal = sqlCommd.ExecuteNonQuery()
        Catch ex As Exception
            'Log the duplicate record for reference.
            objAppContainer.objLogger.WriteAppLog("RefFileParser: Create Statistics: " + ex.Message, _
                                                         Logger.LogLevel.ERROR)
        End Try
    End Sub
End Class
