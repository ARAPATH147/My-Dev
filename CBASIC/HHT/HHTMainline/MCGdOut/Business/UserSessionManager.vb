...
Public Class UserSessionManager
    ...
    Public Function CheckExportDataAvailability() As Boolean
        ...
        Dim dtLastModifiedTime As Date
        Dim dtTimeNow As Date = Now()
        Dim dtDateYest As Date
        Dim dtCheckDate As Date
        Try
            dtDateYest = dtTimeNow.AddDays(-1)
            dtCheckDate = New DateTime(dtDateYest.Year, dtDateYest.Month, _
                                                dtDateYest.Day, _
                                                23, _
                                                0, 0)
            ...
            'If Not CheckForCrashAndRecover() Then
            ' Check if file exists
            If IO.File.Exists(strExportFile) Then
                '@Fix start
                dtLastModifiedTime = IO.File.GetLastWriteTime(strExportFile)
                If dtLastModifiedTime < dtCheckDate Then
...
#If NRF Then
    ...
    Public Function DeleteActiveData() As Boolean
        ...
        Try
            ...
            bTemp = objAppContainer.objActiveFileParser.PurgeActiveData()
            End If
        Catch ex As Exception
...
#If NRF Then
    ...
    Public Function DownloadExportData() As String
        ...
        Try
            ...
            If bNoErrorDisplay Then
                ...
                MessageBox.Show(MessageManager.GetInstance.GetMessage("M64"), _
                                "Connection Error", MessageBoxButtons.OK, MessageBoxIcon.Hand, _
        ...
        Catch ex As Exception
            ...
            Return "-1"
        Finally
            If Not objExDataTransmitter Is Nothing Then
                objExDataTransmitter.EndSession()
            End If
    ...
    Public Function GetActiveData() As Boolean
        ...
        Try
            ...
            ElseIf objAppContainer.bMCFEnabled Then
                objAppContainer.objDataEngine.sConnectAlternateInBatch()
                'Set splash screen message
                objAppContainer.objSplashScreen.ChangeLabelText("Connecting to the Alternate controller...")
                'Instantiate the Socke Connection manager class
                objExDataTransmitter = New ExDataTransmitter()
                'Get the SOR record to be sent.
                strSORRecord = objAppContainer.objExportDataManager.GenerateSOR( _
            ...
            If objAppContainer.objDataEngine.GetUserDetails(strUserEnteredID, objUserInfo) Then
                ...
                If Not objUserInfo Is Nothing Then
                    ...
                    If (strUserEnteredPwd = objUserInfo.Password) Then
                        'Added as part of SFA - restrict the users who are not supervisors   
                        If Not (objUserInfo.StockAccess = Macros.STOCK_SPECIALIST) Then
                ...
                End If
            Else
                MessageBox.Show(MessageManager.GetInstance.GetMessage("M65"), _
                                "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand, _
                                MessageBoxDefaultButton.Button1)
            End If
        Catch ex As Exception
                ...
                If objBatchProcess.BatchProcess(True, m_ActBuildTime) = "1" Then
                    ...
                    Else
                        objAppContainer.objLogger.WriteAppLog( _
                                        ...
                                        Logger.LogLevel.RELEASE)
                        MessageBox.Show(MessageManager.GetInstance.GetMessage("M63"), _
                                        "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand, _
                                        MessageBoxDefaultButton.Button1)
                    End If
                Else
                    objAppContainer.objLogger.WriteAppLog( _
                                        ...
                                        Logger.LogLevel.RELEASE)
                    MessageBox.Show(MessageManager.GetInstance.GetMessage("M62"), _
                                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand, _
                                    MessageBoxDefaultButton.Button1)
                End If
...
#End If
    ...
    Private Sub InsertSOR()
        Try
            ...
            If objAppContainer.objExportDataManager.CreateSOR(objUserInfo.Password) Then
                objAppContainer.objLogger.WriteAppLog( _
...
#If NRF Then
            ...
            If bIsExDataAvailable = True Then
                ...
                If bIsCrashRecovered Then
                    ...
                    MessageBox.Show(MessageManager.GetInstance.GetMessage("M60"), "Error", _
...
#If NRF Then
    ...
    Private Function UpdateDB() As Boolean
        ...
        Try
            ...
            If objAppContainer.objActiveFileParser.ParseFile(Macros.CREDIT) Then
...
#If NRF Then
        ...
        If bIsExportDataDownload Then
            ...
            End If
            'Display message to ask user to download the export data file
            'diaResult = MessageBox.Show(MessageManager.GetInstance.GetMessage("M28"), _
            '                            "Alert", MessageBoxButtons.YesNo, _
            '                            MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
            ' Check whether the user has decided to download the file or not
            Try
                'If (diaResult = Windows.Forms.DialogResult.Yes) Then
                'If downloaded export data file done, Get active data file for next set of operation 
        ...
        ElseIf Not bIsExportDataDownload Then
            'Insert OFF record in the export data file
            If ReadLastLine() = False Then
                objAppContainer.objExportDataManager.CreateOFF(False)
            End If
...
Public Class UserInfo
    ...
    Public user_AccessStock As String
    ''' <summary>
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Property StockAccess() As String
        Get
            Return user_AccessStock
        End Get
        Set(ByVal value As String)
            user_AccessStock = value
        End Set
    End Property
End Class
''' <summary>
...
Public Class BatchProcessor
    ...
    Public Function BatchProcess(ByVal strAction As Boolean, ByVal strActBuildTime As String) As String
        ...
        Try
            ...
            strHostIP = ConfigDataMgr.GetInstance.GetParam(ConfigKey.ACTIVE_IPADDRESS)
    ...
    Private Sub GetControlFile()
        ...
        Try
            ...
            'objAppContainer.objLogger.WriteAppLog("Downloading", Logger.LogLevel.RELEASE)
            ...
            '        MessageBox.Show("Socket status : " + m_Socket.Client.Connected.ToString())
            '    Catch ex As Exception
            '        bConnectAlt = True
            '        MessageBox.Show(ex.ToString())
            '        objAppContainer.objLogger.WriteAppLog("Execption in tcpip conn: " + ex.ToString())
            '    End Try
            '    MessageBox.Show("Host  : " + m_tyOptions.Host)
            '    objAppContainer.objLogger.WriteAppLog("Primary connection status" + m_Socket.Client.Connected.ToString(), Logger.LogLevel.RELEASE)
            '    If Not m_Socket.Client.Connected Then
            '        bConnectAlt = True
            '    End If
            '    'If there is no conenction to primary
            '    If bConnectAlt Then
            ...
            '    End If
            '    objAppContainer.objLogger.WriteAppLog("Alternate IP  " + objAppContainer.strActiveIP, Logger.LogLevel.RELEASE)
            'End If
