...
\***
\***  Ver F     David Griffiths                                    12 July 2017
\***            There is now a new block from sale on the irf in indicat1 bit 5
\***            functionality needed to remove block on this flag.
\***
\***  Ver G     Arjun  Muraleedharan                               09 Oct  2017
\***            At present there is no mechanism to track if an item block is
\***            failed or not other than manually checking it. It becomes 
\***            increasingly difficult to identify if there hundreads of item 
\***            block at one go. Added some error trapping in the code so that 
\***            it will tell the number of failes and failed item code at the 
\***            end
\*******************************************************************************
\*******************************************************************************
\*******************************************************************************
...
INTEGER*2 I%,                             \
          ...
          ERR.SESS.NUM%,                  \
          EXPCTD.NO.CODES%,               \                                  !BCS
          FAILED.ARRAY.INDEX%,            \                                  !GAM
          FAILED.ARRAY.MAX%,              \ !                                !GAM
          FAILED.STATUS%,                 \                                  !GAM        
          INDEX%,                         \                                  !BCS
          MESSAGE.NUMBER%,                \
          BLOCKITM.SESS.NUM%,             \
...
STRING    ADXSERVE.DATA$,                 \
          ...
          BLOCKITM.PATH$,                 \
          FAILED.BOOTS.CODES$(1),         \ ! Failed item codes              !GAM
          INPUT.FILLER$,                  \
          INPUT.BOOTS.CODE$,              \
          MSG$,                           \                                  !DCS
          MODULE$,                        \
...
SUB READ.AND.UPDATE.IRF
    ...
    IF RC% = 0 THEN BEGIN ! Item exists on IRF file
        ...
        ENDIF ELSE BEGIN ! "NO"
            ...
            ! Reset withdrawn block from sale bit flag 5             ! FDG
            IRF.INDICAT1% = IRF.INDICAT1% AND 11101111B              ! FDG
        ENDIF
    ...
    ENDIF ELSE BEGIN
        ...
        ! There is no tracking to know if IRF read is failed, it makes difficult to track     !GAM
        ! if there are large number of items to block or unblock. Below check will make       !GAM
        ! Sure failures are reported with item code status at the end of the file             !GAM

        IF CURRENT.IDF.CODE$ <> INPUT.BOOTS.CODE$ THEN BEGIN                                  !GAM
            FAILED.STATUS% = FAILED.STATUS% + 1                                               !GAM
            CURRENT.IDF.CODE$ = INPUT.BOOTS.CODE$                                             !GAM
            FAILED.BOOTS.CODES$(FAILED.STATUS%) = INPUT.BOOTS.CODE$                           !GAM
        ENDIF                                                                                 !GAM
...
INITIALISATION:
      ...
      FAILED.STATUS%       = 0                                                !GAM
      FAILED.ARRAY.MAX%    = 2000                                             !GAM
      FAILED.ARRAY.INDEX%  = 1                                                !GAM
      DIM FAILED.BOOTS.CODES$(FAILED.ARRAY.MAX%)                              !GAM
...
GET.NO.OF.BARCODES:
    ...
    ENDIF ELSE BEGIN
        ...
        FAILED.STATUS% = FAILED.STATUS% + 1                             !GAM
        FAILED.BOOTS.CODES$(FAILED.STATUS%) = INPUT.BOOTS.CODE$         !GAM
        FAILED.ARRAY.INDEX% = FAILED.ARRAY.INDEX% + 1                   !GAM
    ENDIF
...
TERMINATION:
    ...
    IF FAILED.STATUS% = 0 THEN BEGIN                                    !GAM
    CALL DO.MESSAGE("Completed Successfully")                           
    ENDIF                                                               !GAM


    IF FAILED.STATUS% > 0 THEN BEGIN                                    !GAM
        CALL DO.MESSAGE( STR$(FAILED.STATUS%) +" Items have either " +  \GAM
               + "IDF or IRF read issues please check the status")      !GAM
        FAILED.ARRAY.INDEX% = 1                                         !GAM
        FOR FAILED.ARRAY.INDEX% = 1 TO FAILED.STATUS%                   !GAM

            CALL DO.MESSAGE(LEFT$(FAILED.BOOTS.CODES$                   \GAM
                                            (FAILED.ARRAY.INDEX%),7))   !GAM
        NEXT FAILED.ARRAY.INDEX%                                        !GAM
    ENDIF                                                               !GAM
