...
\***
\***  Ver F     Arjun Muraleedharan                                 18 Feb 2017
\***            Found a defect with the code. Application will not block the 
\***            item codes if there is only one bar code.  Relinked the application
\***            and made sure bit flag is taken properly.Also introduced
\***            more error trapping in the code so that failed item status
\***            and count will be displayed on the log file as the display screen.
\***            
\***
\*******************************************************************************
\*******************************************************************************
...
INTEGER*2 I%,                             \
          ...
          ERR.SESS.NUM%,                  \
          EXPCTD.NO.CODES%,               \                                  !BCS
          FAILED.ARRAY.INDEX%,            \                                  !FAM
          FAILED.ARRAY.MAX%,              \ !                                !FAM
          FAILED.STATUS%,                 \ !                                !FAM
          INDEX%,                         \                                  !BCS
          MESSAGE.NUMBER%,                \
          BLOCKITM.SESS.NUM%,             \
...
STRING    ADXSERVE.DATA$,                 \
          ...
          BLOCKITM.PATH$,                 \
          FAILED.BOOTS.CODES$(1),         \ ! Failed item codes              !FAM
          INPUT.FILLER$,                  \
          INPUT.BOOTS.CODE$,              \
          MSG$,                           \                                  !DCS
          MODULE$,                        \       
...
SUB READ.AND.UPDATE.IRF
    ...
    ENDIF ELSE BEGIN
        ...
        ! There is no tracking to know if IRF read is failed, it makes difficult to track     !FAM
        ! if there are large number of items to block or unblock. Below check will make       !FAM
        ! Sure failures are reported with item code status at the end of the file             !FAM

        IF CURRENT.IDF.CODE$ <> INPUT.BOOTS.CODE$ THEN BEGIN                                  !FAM
            FAILED.STATUS% = FAILED.STATUS% + 1                                               !FAM
            CURRENT.IDF.CODE$ = INPUT.BOOTS.CODE$                                             !FAM
            FAILED.BOOTS.CODES$(FAILED.STATUS%) = INPUT.BOOTS.CODE$                           !FAM
...
INITIALISATION:
      ...
      VERSION$ = "6.0  02/17/17"                                             !DCS !ECS !FAM
      ...
      FAILED.STATUS%       = 0                                                !FAM
      FAILED.ARRAY.MAX%    = 2000                                             !FAM
      FAILED.ARRAY.INDEX%  = 1                                                !FAM
      DIM FAILED.BOOTS.CODES$(FAILED.ARRAY.MAX%)                              !FAM
...
GET.NO.OF.BARCODES:
    ...
    ENDIF ELSE BEGIN                                                                          
        ...
        FAILED.STATUS% = FAILED.STATUS% + 1                             !FAM         
        FAILED.BOOTS.CODES$(FAILED.STATUS%) = INPUT.BOOTS.CODE$         !FAM
        FAILED.ARRAY.INDEX% = FAILED.ARRAY.INDEX% + 1                   !FAM
    ENDIF
...
TERMINATION:
    ...
    IF FAILED.STATUS% = 0 THEN BEGIN                                    !FAM
       CALL DO.MESSAGE("Completed Successfully")                        !FAM
    ENDIF                                                               !FAM


    IF FAILED.STATUS% > 0 THEN BEGIN                                    !FAM
        CALL DO.MESSAGE( STR$(FAILED.STATUS%) +" Items have either " +  \FAM
               + "IDF or IRF read issues please check the status")      !FAM
        FAILED.ARRAY.INDEX% = 1                                         !FAM
        FOR FAILED.ARRAY.INDEX% = 1 TO FAILED.STATUS%                   !FAM

            CALL DO.MESSAGE(LEFT$(FAILED.BOOTS.CODES$                   \FAM
                                            (FAILED.ARRAY.INDEX%),7))   !FAM
        NEXT FAILED.ARRAY.INDEX%                                        !FAM
    ENDIF                                                               !FAM
