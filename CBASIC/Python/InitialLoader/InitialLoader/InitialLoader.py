...
def CreateOneFile():      
    ...
    for file in os.listdir(Filepath): 
        ...
        with open(Fullpath,'rb') as f: 
            I = 0
            for line in f:           
                SeventyFive =  bool('Response :75:'.encode('utf-8') in line)
                if SeventyFive == True:                    
                    NewLine = re.split("[:,]".encode('utf-8'),line)
                    if (NewLine[12] != "" and NewLine[12] != '0'):           
                        LineEntry  = str(NewLine[12] )+ str(linux_line_ending)    
            ...
            if len(Valid_Entry) != 0:
                ...
                PrintLine = "Boots" + "," + file[0:4] + ",,"+ file[0:4]+ "%" +"," + "" + str(Valid_Entry) + "/r/n"
                print(PrintLine)
                with open("c:/temp/just.txt", 'wb') as d:       
                     
                    d.write(PrintLine.encode('utf-8'))
